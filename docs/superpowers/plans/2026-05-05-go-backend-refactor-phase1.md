...
- **WriteError migration is partial** (Task 9 follow-up). The `httperr` package's switch is now complete — every captured (status, code) pair from `error_codes_observed.md` plus every documented gap has a matching arm. But several handlers still use inline `writeJSON`/`jsonError` rather than `httperr.WriteError`:
  - **Auth slice (fully un-migrated)**: `auth/adapters/http/handlers.go` lines 60, 79, 84, 92, 97, 102 + `router.go:74,80,84` + `middleware.go:63` emit `unauthorized`, `unknown_provider`, `bad_state`, `exchange_failed`, `upsert_failed`, `sign_failed` inline. The `httperr` switch now has arms for all of these (using `authdomain.ErrUnknownProvider`, `authdomain.ErrBadState`, `authdomain.ErrExchangeFailed`, `httperr.ErrUnauthorized`).
  - **Image handler partial**: `image/adapters/http/handler.go` lines 39, 44, 48, 53, 58, 63, 72, 81 emit `not_found`, `unsupported_media_type`, `bad_multipart`, `manifest_required`, `file_count_mismatch`, `open_file`, `decode_failed` inline. The switch has arms for all except `bad_multipart` (which isn't in the captured error_codes_observed.md — it may be removable or re-aliased to one of the documented codes).
  - **Artwork inline `not_found`** at `artwork/adapters/http/router.go:101,172,223` — guards using `if err != nil || <ownership-check>`. Switch already has the arm (`artworkdomain.ErrNotFound`). Three call sites to migrate.
  
  **Migration recipe per handler**: replace `writeJSON(w, <status>, map[string]string{"error":"<code>"})` with `httperr.WriteError(w, h.log, <sentinel>)`. Verify byte-strict via `make test-contract` after each handler. The `httperr` package builds the same JSON envelope, so byte equivalence holds. The watch-out: `writeJSON` may have written extra fields (e.g., `Message`) that need preserving in the WriteError sentinel path.
