...
- Artwork detail page (`/art/<publicId>`) with full-resolution images, artist name, publish date, tags
...
CREATE TABLE artworks (
  id             bigserial PRIMARY KEY,
  public_id      text UNIQUE NOT NULL,                      -- nanoid(10)
  user_id        bigint NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  title          text NOT NULL,
  description    text,
  visibility     text NOT NULL CHECK (visibility IN ('public','private')),
  cover_image_id bigint,                                    -- FK below
  created_at     timestamptz NOT NULL DEFAULT now(),
...
- `public_id` is a 10-char nanoid generated in app code, used in URLs. Internal joins use `id`.
...
- `storage_key` includes the visibility prefix: `public/<public_id>/<position>.<ext>` or `private/...`. The prefix encodes the access policy for the Worker.
...
GET    /artworks/:public_id          → detail; 404 if private and not owner
...
The auth package exposes a `Provider` interface so additional providers (Facebook, Instagram) plug in later without touching handlers.
...
5. Build storage key: `<visibility>/<public_id>/<position>.<ext>` where `position` is the next free index for this artwork.
...
app/
  ...
  art/[publicId]/page.tsx      - artwork detail
...
elif path starts with "/private/":
    sig = query["sig"]
    exp = query["exp"]
    if missing or now > exp:
        return 401
    expected = hex(HMAC_SHA256(secret, path + "|" + exp))
    if sig != expected:
        return 401
    pass through to R2
...
- **API** returns an *origin* URL — the CDN host plus the storage path, with `sig` + `exp` query params for private content. No transform options.
- **Frontend** (`cf-loader`) wraps that URL with `/cdn-cgi/image/width=<W>,format=auto,quality=85/...` per render, choosing `<W>` from the requested rendering size.
...
func (s *Service) PublicImageURL(key string) string {
    return "https://cdn.example.com/" + key
}

func (s *Service) PrivateImageURL(key string) string {
    exp := time.Now().Add(5 * time.Minute).Unix()
    mac := hmac.New(sha256.New, s.signingKey)
    mac.Write([]byte(fmt.Sprintf("%s|%d", key, exp)))
    sig := hex.EncodeToString(mac.Sum(nil))
    return fmt.Sprintf("https://cdn.example.com/%s?sig=%s&exp=%d", key, sig, exp)
}
```

```ts
// Frontend: cf-loader.ts — wraps origin URL with the transform.
export function cfLoader({ src, width, quality }: { src: string; width: number; quality?: number }) {
  ...
  // src is something like "https://cdn.example.com/private/abc/0.jpg?sig=X&exp=Y"
  // Cloudflare's transform preserves the query string so the Worker still sees sig/exp.
  return src.replace("https://cdn.example.com/", `https://cdn.example.com/cdn-cgi/image/${opts}/`);
}
```

The Worker validates `sig`/`exp` against the same `signingKey` the API holds. Five-minute TTL is the trade-off between leakage window (a leaked URL is valid for ≤5 min) and re-fetch frequency (`next/image` re-validation handles refresh transparently).

### 8.4 Information leakage rules
...
| Privacy | All eight cases in §8.5 | Dedicated `internal/artwork/privacy_test.go` |
...
- All eight privacy tests in §8.5 pass.
