// api/internal/auth/flow_test.go
//
...
func TestStart_RealProvider_IncludesClientIDInRedirect(t *testing.T) {
	...
	provider := auth.NewGoogleProviderForTest(
	...
	h := auth.StartHandler(map[string]auth.Provider{"google": provider}, auth.CookieOpts{})
...
func TestFullOAuthFlow_StartThenCallback(t *testing.T) {
	...
	providers := map[string]auth.Provider{"google": provider}

	// --- Phase 1: GET /auth/google/start ---------------------------------
	startH := auth.StartHandler(providers, cookieOpts)
	...
	callbackH := auth.CallbackHandler(providers, repo, jwts, "https://app.local/", cookieOpts)
